...
// Full area data reconstructed from user's memory (2026-04-23 restore session)
const AREAS = [
  {
    ...
    current_stage:
      "Right now this area is in a shaping stage where you are defining the business growth path, clarifying the offer, and identifying which assets and systems will make ASL more visible, premium, and scalable.",
    why_it_matters:
  ...
  },
  {
    ...
    why_it_matters:
      "This area is about building a client-facing project portal where renovation clients can log in and see their project through a personalized dashboard.",
    now_focus:
  ...
  },
  {
    ...
    later_focus:
      "Eventually, I'd love this area to become a small catalogue of books and digital products that feel aligned with how I actually live and work, and that support the other areas instead of competing with them.",
    milestones: [
  ...
  },
  {
    ...
    current_stage: null,
    why_it_matters:
      "It matters because my art is a real part of who I am, and I do not want it to get buried under my other business and product work. A separate strategic area helps keep an important identity visible instead of letting it disappear behind louder priorities.",
    now_focus:
...
async function restore() {
  ...
  try {
    ...
    // GUARD: only run the one-time restore if the areas table is empty.
    ...
    if (existingAreas.rows.length > 0 && process.env.QUEST_FORCE_RESTORE !== "1") {
      console.log("[restore] areas already exist — skipping restore to preserve user edits.");
    ...
    for (const p of AREAS) {
      // Upsert by name: if area exists, update it; otherwise insert
      const existing = await client.query("SELECT id FROM areas WHERE name = $1", [p.name]);
      ...
      for (let i = 0; i < p.milestones.length; i++) {
        ...
        const existingM = await client.query(
          "SELECT id FROM milestones WHERE area_id = $1 AND title = $2",
          [areaId, m.title]
        );
        ...
        await client.query(
          `INSERT INTO milestones (area_id, title, status, priority, next_action, sort_order)
           VALUES ($1, $2, $3, $4, $5, $6)`,
          [areaId, m.title, m.status, m.priority, m.next_action, i]
        );
        milestonesInserted++;
      }
