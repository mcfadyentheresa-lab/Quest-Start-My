...
function reviewToReflection(review: MonthlyReview | null): ReflectionValues {
  return {
    moved: review?.whatMoved ?? "",
    stuck: review?.whatDelayed ?? "",
    drop: review?.whatToPause ?? "",
    nextFocus: (review?.topPrioritiesNextMonth ?? []).filter(Boolean).join("\n"),
  };
}
...
export default function ReviewPage() {
  ...
  const reflection = reviewToReflection(existingReview);
            ...
            {isLoading ? (
              ...
              >
                <ReflectionForm
                  ...
                  saving={isSaving}
                  saveLabel={existingReview ? "Update reflection" : "Save reflection"}
                />
              </motion.div>
            )}
          </motion.div>
